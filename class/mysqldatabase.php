 ...
 * @subpackage database
 */
class StillsMySQLDatabase extends StillsDatabase
{
    ...
    public function connect($selectdb = true)
    {
        ...
        if (DB_STILLS_PERS == true) {
            $this->conn = @mysql_pconnect(DB_STILLS_HOST, DB_STILLS_USER, DB_STILLS_PASS);
        } else {
            $this->conn = @mysql_connect(DB_STILLS_HOST, DB_STILLS_USER, DB_STILLS_PASS);
        }
        ...
        if (!$this->conn) {
			...
			if ($selectdb != false) {
				if (!mysql_select_db(DB_STILLS_NAME)) {
					die('Could not select: ' . mysql_error());
				}
			}
        if (!isset($db_charset_set) && defined('DB_STILLS_CHAR') && DB_STILLS_CHAR) {
            $this->queryF("SET NAMES '" . DB_STILLS_CHAR . "'");
        }
        $db_charset_set = 1;
    ...
    /**
     * StillsMySQLDatabase::fetchObjected()
     *
     * @param resource $result
 ...
 *
 * @author Kazumi Ono <onokazu@Stills.org>
 * @copyright copyright (c) 2000-2003 Stills.org
 * @package feeds
 * @subpackage spline
 */
class StillsMySQLDatabaseSafe extends StillsMySQLDatabase
{
    /**
     * perform a query on the database
     *
 ...
 * @subpackage spline
 */
class StillsMySQLDatabaseProxy extends StillsMySQLDatabase
{
    /**
     * perform a query on the database
     *
